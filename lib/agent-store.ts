...
\`\`\`tsx
// FALSCH! Vertikale Liste!
{days.map(day => <div>{day}</div>)}

// FALSCH! Keine Grid-Klasse!
<div>{days.map(...)}</div>

// FALSCH! flex-col statt grid!
<div className="flex flex-col">{days.map(...)}</div>
\`\`\`

**✅ SO MUSS KORREKTER KALENDER-CODE AUSSEHEN:**
\`\`\`tsx
// RICHTIG! 7-Spalten Grid!
<div className="grid grid-cols-7 gap-1">
  {days.map(day => (
    <div key={day} className="aspect-square p-2 rounded-xl border border-zinc-800">
      {day}
    </div>
  ))}
</div>
...
**⛔ ABSOLUT VERBOTEN:**
- Vertikale Listen von Zahlen/Tagen
...
// ✅ RICHTIG:
<div className="p-4 bg-zinc-900/50 border border-zinc-800 rounded-xl">{text}</div>
{items.map(i => (
...
## 🎨 HEROUI FRAMEWORK - PREMIUM UI-KOMPONENTEN!

**HeroUI ist ein modernes React UI Framework mit professionellen Komponenten.**
**VERWENDE HEROUI für besseres Design!**

### PFLICHT-DATEIEN FÜR HEROUI + TAILWIND:

\`\`\`json
// filepath: package.json (Dependencies hinzufügen)
{
  "dependencies": {
    "@heroui/react": "^2.7.0",
...
### 🚀 HEROUI KOMPONENTEN - VERWENDE DIESE!
...
// Beispiel Button:
<Button color="primary" variant="shadow">Klick mich</Button>
<Button color="secondary" variant="bordered">Abbrechen</Button>
<Button color="danger" variant="flat">Löschen</Button>
...
<Card className="max-w-md">
  <CardHeader className="flex gap-3">
    ...
    <div className="flex flex-col">
      <p className="text-md">Titel</p>
      <p className="text-small text-default-500">Beschreibung</p>
    </div>
  </CardHeader>
  <CardBody>
    <p>Inhalt der Card</p>
  ...
  <CardFooter>
    <Button color="primary">Aktion</Button>
  </CardFooter>
</Card>
...
<Button onPress={onOpen}>Modal öffnen</Button>
<Modal isOpen={isOpen} onClose={onClose}>
  <ModalContent>
    ...
    <ModalFooter>
      ...
      <Button color="primary" onPress={onClose}>Speichern</Button>
    </ModalFooter>
  </ModalContent>
</Modal>

// Beispiel Table:
<Table aria-label="Beispiel Tabelle">
  <TableHeader>
  ...
  <TableBody>
    <TableRow key="1">
      ...
      <TableCell><Chip color="success">Aktiv</Chip></TableCell>
    </TableRow>
  </TableBody>
</Table>

// Beispiel Tabs:
<Tabs aria-label="Optionen">
  <Tab key="tab1" title="Tab 1">Inhalt 1</Tab>
  <Tab key="tab2" title="Tab 2">Inhalt 2</Tab>
</Tabs>
\`\`\`

### ⚠️ WICHTIG BEI HEROUI:
- IMMER \`app/providers.tsx\` mit HeroUIProvider erstellen!
- IMMER \`@heroui/react\` in package.json Dependencies!
- IMMER \`heroui()\` Plugin in tailwind.config.js!
- Button: \`onPress\` statt \`onClick\` verwenden!
\`\`\`
